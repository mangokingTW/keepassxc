...
static int server_is_our_application(HANDLE pipe)
{
    ...
    if (!GetModuleFileNameW(NULL, expected, MAX_PATH)) {
        return 0;
    }
...
static int target_is_credential_dialog(HWND window)
{
    ...
    wchar_t expected[MAX_PATH] = {0};
    const UINT length = GetSystemDirectoryW(expected, MAX_PATH);
    if (length == 0 || length >= MAX_PATH) {
        return 0;
    }
    if (_snwprintf_s(expected + length, MAX_PATH - length, _TRUNCATE, L"\\CredentialUIBroker.exe") < 0) {
        return 0;
    }
...
/* Shift, Ctrl, Alt and both Windows keys, released unconditionally.
 *
 * A sequence types modifiers as separate down and up records, so aborting
 * between the two leaves one held down for the whole desktop -- every
 * subsequent keystroke the user makes arrives shifted. The old ::SendInput
 * path could not do this because it never abandoned a sequence part-way. */
static void release_modifiers(void)
{
    static const WORD keys[] = {VK_SHIFT,
                                VK_CONTROL,
                                VK_MENU,
                                VK_LSHIFT,
                                VK_RSHIFT,
