...
# The path comes from the caller, as an argument first. It was hardcoded once,
# and because the replacement that was supposed to fix it silently matched
# nothing, the tests read an empty file and reported a working mechanism as
# broken. The environment variable went the same way for the same reason: this
# is now started at medium integrity through a launcher that builds the child a
# fresh environment block from the user token, so a variable set by the caller
# does not arrive, and the fallback path was written instead -- while the
# harness watched the path it had asked for and saw nothing.
result_path = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.environ.get("CREDUI_PROBE_RESULT", r"C:\Users\Public\credui_probe_result.txt")
)
print(f"writing the result to {result_path}", flush=True)
