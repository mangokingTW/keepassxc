...
| `verify_refusals.py` | the other direction: that the helper is *not* started when the setting is off, when KeePassXC sits in a location Windows would not grant uiAccess from, or when the target is not a credential dialog. Each case is paired with a positive control in the same run, because a run of refusals alone cannot tell "correctly refused" from "nothing worked" |
...
- KeePassXC installed under a path a standard user cannot write, such as
  `%ProgramFiles%`, with `keepassxc-uiaccess-helper.exe` beside it (that is
  where `cmake --install` puts it) and signed with a certificate the machine
  trusts -- see the workflow. Windows refuses uiAccess otherwise, and the
  patched build then behaves like the unpatched one, which is the intended
  fallback
