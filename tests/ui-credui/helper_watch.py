...
# Windows uses values between these, and they matter here: the credential dialog
# measures 0x200a and a granted uiAccess helper 0x2010. Naming only the well-known
# levels is what hid that -- the raw value is printed for anything unnamed.
NAMES = {0x1000: "Low", 0x2000: "Medium", 0x2010: "uiAccess", 0x2100: "MediumPlus",
         0x3000: "High", 0x4000: "System"}
TARGETS = ("keepassxc-uiaccess-helper",)
...
def integrity_rid(pid: int) -> int | None:
    """The raw mandatory level, for comparing. None when it cannot be read.

    Raw because the labels cannot be ordered: a check that compared them put
    "0x200a (...)" below "Medium (0x2000)" and never fired.
    """
...
def integrity(pid: int) -> str:
    """The same reading as a label, for the record.

    "Could not read it" stays distinguishable from "Medium": an earlier round
    used TokenElevation, which answers a different question, and reported a
    de-elevated child as privileged.
    """
    rid = integrity_rid(pid)
    if rid is None:
